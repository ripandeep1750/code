...
# Define the clusters and features
clusters = ['Cluster 1', 'Cluster 2', 'Cluster 3', 'Cluster 4', 'Cluster 5']
features_list = ['5 Features', '10 Features', '20 Features', '30 Features']
# Create an empty DataFrame to store the results
results_df = pd.DataFrame(columns=clusters, index=features_list)
results_df.columns.name = 'KNN for PCA'
# Loop through each cluster and feature combination
for cluster in clusters:
    for i in range(len(features_list)):
        # Define the PCA data based on the cluster
        ...
        results_df.at[features_list[i], cluster] = accuracy
...
table_data = pd.DataFrame(columns=['Feature', 'Cluster 1', 'Cluster 2', 'Cluster 3', 'Cluster 4', 'Cluster 5'])
# Iterate over the features
for i, feature in enumerate(features_list):
    feature_accuracy = []
    for cluster_num in range(1, 6):
        # Get the corresponding data and labels for the cluster
        mi_data = globals()['mi_data_clus' + str(cluster_num)]
        X = globals()['X' + str(cluster_num)]
        y = globals()['y' + str(cluster_num)]
        # Encode the labels
        encoded_labels = labelEn.fit_transform(y.values)
        # Extract the best features for the current feature and cluster
        best_features = extract_best_features(mi_data, X.columns.values, n=n_features[i])
        drop_these = list(set(X.columns.values) - set(best_features[:, 1]))
        data_mi = X.drop(drop_these, axis=1, inplace=False)
        # Split the data into training and testing sets
        X_train, X_test, y_train, y_test = train_test_split(data_mi, encoded_labels, test_size=0.2, shuffle=False)
        # Train the KNN classifier
        neigh = KNeighborsClassifier(n_neighbors=63, n_jobs=-1)
        neigh.fit(X_train, y_train)
        # Predict the results
        y_pred = neigh.predict(X_test)
        # Calculate and append the accuracy to the feature_accuracy list
        feature_accuracy.append(100 * accuracy_score(y_pred, y_test))
    # Add a row for the current feature in the table_data dataframe
    table_data.loc[i] = [feature] + feature_accuracy
# Set the 'Feature' column as the index of the dataframe
# table_data.set_index('Feature', inplace=True)
# Add the table heading
# table_data.columns.name = 'Cluster'
# Print the table
print(table_data)
...
import pandas as pd
# Create an empty dataframe for the table
table_data = pd.DataFrame(columns=['Cluster 1', 'Cluster 2', 'Cluster 3', 'Cluster 4', 'Cluster 5'], 
                          index=['5 Features', '10 Features', '20 Features', '30 Features'])
# For Cluster 1
encoded_labels = labelEn.fit_transform(y1.values)
features_list = ['5 Features', '10 Features', '20 Features', '30 Features']
for i in range(len(features_list)):
    ...
    neigh.fit(X_train, y_train)
    y_pred = neigh.predict(X_test)
    table_data.loc[features_list[i], 'Cluster 1'] = 100 * accuracy_score(y_pred, y_test)
...
for i in range(len(features_list)):
    ...
    neigh.fit(X_train, y_train)
    y_pred = neigh.predict(X_test)
    table_data.loc[features_list[i], 'Cluster 2'] = 100 * accuracy_score(y_pred, y_test)
...
for i in range(len(features_list)):
    ...
    neigh.fit(X_train, y_train)
    y_pred = neigh.predict(X_test)
    table_data.loc[features_list[i], 'Cluster 3'] = 100 * accuracy_score(y_pred, y_test)
...
for i in range(len(features_list)):
    ...
    neigh.fit(X_train, y_train)
    y_pred = neigh.predict(X_test)
    table_data.loc[features_list[i], 'Cluster 4'] = 100 * accuracy_score(y_pred, y_test)
...
for i in range(len(features_list)):
    ...
    neigh.fit(X_train, y_train)
    y_pred = neigh.predict(X_test)
    table_data.loc[features_list[i], 'Cluster 5'] = 100 * accuracy_score(y_pred, y_test)
...
for j in range(len(features_list)):
        drop_these = list(set(X5.columns.values) - set(rfe_features_clus5[j]))
        data_clus_rfe = X5.drop(drop_these, axis=1, inplace=False)
        X_train, X_test, y_train, y_test = train_test_split(data_clus_rfe, encoded_labels, test_size=0.2, shuffle=False)
        neigh = KNeighborsClassifier(n_neighbors=63, n_jobs=-1)
        neigh.fit(X_train, y_train)
        y_pred = neigh.predict(X_test)
        table_data.loc[features_list[j], 'Cluster 5'] = 100 * accuracy_score(y_pred, y_test)
